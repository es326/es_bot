	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Adios@${num.split('@')[0]}Espero que não volte hehe 👋`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				...
				only: {
					group: '❌ O idiota nos não estamos em um grupo ou seja só pode ser usado em grupos! ❌',
					ownerG: '❌ Este comando só pode ser usado pelo grupo proprietário! ❌',
					ownerB: '❌ Você não é proprietário cale a boca! ❌',
					admin: '❌ Você não é adimin então fica quieto membro comum! ❌',
					Badmin: '❌ O bot ainda não é adimin seu idiota! ❌'
				}
			}
			...
			switch(command) {
				...
				case 'bot':
					...
					teks = `*Nome do bot* : ${me.name}\n*Número do bot* : @${me.jid.split('@')[0]}\n*Prefix* : ${prefix}\n*Total de contantos bloqueados* : ${blocked.length}\n*O bot está ativo desde:* : ${kyun(uptime)}`
				...
				case 'sticker':
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						await ffmpeg(`./${media}`)
							...
							.on('error', function (err) {
								...
								reply(`❌ fodeu, falha ao converter $ {type} em adesivo`)
							})
							.on('end', function () {
								console.log('Finish')
								client.sendMessage(from, fs.readFileSync(ran), sticker, {quoted: mek})
					...
					break
				case 'gtts':
					if (args.length < 1) return client.sendMessage(from, 'Onde está o código do idioma, seu burro?', text, {quoted: mek})
           ...
           case 'belle3':
             ...
             break
               ...
               case 'randomhentai':
                    ...
                    break
                case 'criarbot':
		            if (args.length < 1) return reply ('pergunte ao meu criador hehe')
		            break
                case 'Oi':
		            if (args.length < 1) return reply ('oieee')
		            break
                case 'dono':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTBMl1QL6Fc3MPQlgiuavOT716WsdRxYvDc_A&usqp=CAU`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '*NOME:* Es\n*wpp:* wa.me/+5575998855437\n'})
					break       
				case 'memeindo':
					memein = await kagApi.memeindo()
				...
				case 'hilih':
					...
					break
				case 'yt2mp3':
					if (args.length < 1) return reply('Onde está o url, caralho?')
					...
					break
				case 'ytsearch':
					if (args.length < 1) return reply('O que você está procurando? Dick?')
				...
				case 'tiktokstalk':
					try {
						if (args.length < 1) return client.sendMessage(from, 'Onde está o negócio dele? ', text, {quoted: mek})
				...
				case 'tstiker':
				case 'tsticker':
					if (args.length < 1) return reply('Não é possível, o texto está onde?')
					ranp = getRandom('.png')
					rano = getRandom('.webp')
				...
				case 'bc':
					...
					if (mentioned.length > 1) {
						...
						mentions(`Berhasil Promote @${mentioned[0].split('@')[0]} é um adm o respeite!`, mentioned, true)
				...
				case 'demote':
					...
					if (mentioned.length > 1) {
						...
						mentions(`Berhasil Demote @${mentioned[0].split('@')[0]} é um membro comum do grupo, pode zuar a vontade!`, mentioned, true)
				...
				case 'simih':
					...
					if (Number(args[0]) === 1) {
						...
						reply('Ativado com sucesso o modo simi neste grupo começou a baderna✔️')
					} else if (Number(args[0]) === 0) {
						samih.splice(from, 1)
						fs.writeFileSync('./src/simi.json', JSON.stringify(samih))
						reply('Desativando o modo simi com sucesso neste grupo acabou a baderna ✔️')
					} else {
				...
				case 'welcome':
					...
					if (Number(args[0]) === 1) {
						...
						reply('Ativou com sucesso o recurso de boas-vindas neste grupo ieeeeh✔️')
					} else if (Number(args[0]) === 0) {
				...
				case 'clone':
					if (!isGroup) return reply(mess.only.group)
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (args.length < 1) return reply('Tag o alvo que você deseja clonar')
